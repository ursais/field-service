...
class TestFSMActivity(TransactionCase):
    ...
    def test_fsm_activity(self):
        ...
        order = f.save()
        order_id = order.id
        self.Activity.create(self.get_activity_vals('Activity 1',
                                                    False,
                                                    'Ref 1',
                                                    order_id))
        self.Activity.create(self.get_activity_vals('Activity 2',
                                                    False,
                                                    'Ref 2',
                                                    order_id))
        self.Activity.create(self.get_activity_vals('Activity 3',
                                                    True,
                                                    'Ref 3',
                                                    order_id))
        # Test action_done()
        order.order_activity_ids[0].action_done()
        self.assertEqual(order.order_activity_ids[0].
                         completed_on.replace(microsecond=0),
                         datetime.now().replace(microsecond=0))
        self.assertEqual(order.order_activity_ids[0].completed_by,
                         self.env.user)
        self.assertEqual(order.order_activity_ids[0].state,
                         'done')
        # Test action_cancel()
        order.order_activity_ids[1].action_cancel()
        self.assertEqual(order.order_activity_ids[1].state,
                         'cancel')
        ...
        order.action_complete()
        self.assertEqual(order.stage_id.id,
